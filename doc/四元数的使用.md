...
注意一点：由于机器人和计算机图形学中的坐标系定义不一致导致对yaw、pitch、roll定义的轴也不一致


### 1. **航空/航海/机器人学（ENU/NED 坐标系）**
在这些领域，常用的是：
- **X 指向东（East）**
- **Y 指向北（North）**
- **Z 指向上（Up）或向下（Down）**
...
例如：
- **ENU（East-North-Up）**：X=东，Y=北，Z=上
- **NED（North-East-Down）**：X=北，Y=东，Z=下（常用于飞行器）

在这种约定下：
- **Yaw（偏航） = 绕 Z 轴（垂直轴）旋转**
  - 因为 Z 是“上下”方向，是竖直轴
  - Yaw 表示“朝哪个水平方向”，自然绕竖直轴转
...
> 🛩️ 举例：一架飞机在地面，机头指向正北（Y 轴）。  
> - Yaw = 0°：朝北  
> - Yaw = 90°：朝东（绕 Z 轴右转）

---

### 2. **计算机图形学（OpenGL / DirectX / 游戏引擎）**
在 3D 图形中，更常见的是：
- **Y 轴为“世界向上”（World Up）**
- **X 轴为右，Z 轴为前或后**
...
| 系统        | X     | Y     | Z       | 默认相机朝向 |
|-------------|-------|-------|---------|--------------|
| OpenGL      | 右    | 上    | **从屏幕向外** | **看 -Z**     |
| DirectX     | 右    | 上    | **进入屏幕**   | **看 +Z**     |
| Unity       | 右    | 上    | 前（+Z）      | 看 +Z         |
| Unreal      | 前（+X）| 右（+Y）| 上（+Z）   | 看 +X         |
